...
#
# install-user-env.sh
# Phase 2: Install vim-starter environment locally for current user (RHEL 8)
# Run this script as your regular user: ./install-user-env.sh
#
...
# Version variables (matching Dockerfile.ubi8)
RUST_VERSION="1.95"
NEOVIM_TAG="v0.12.2"
LAZYGIT_TAG="v0.61.1"
...
if [ "$EUID" -eq 0 ]; then 
    ...
    echo -e "${RED}Run as your regular user: ./install-user-env.sh${NC}"
    exit 1
fi
...
if [ ${#MISSING_DEPS[@]} -ne 0 ]; then
    ...
    echo -e "${GREEN}  sudo ./install-system-deps.sh${NC}"
    exit 1
fi
...
echo ""
echo -e "${BLUE}========================================${NC}"
echo -e "${BLUE}Step 5/9: Installing ripgrep ${RIPGREP_VERSION}${NC}"
echo -e "${BLUE}========================================${NC}"
if command -v "$LOCAL_BIN/rg" &> /dev/null; then
    echo -e "${YELLOW}ripgrep already installed${NC}"
else
    echo -e "${YELLOW}Downloading ripgrep binary...${NC}"
    cd "$BUILD_DIR"
    ...
    cp ripgrep-${RIPGREP_VERSION}-x86_64-unknown-linux-musl/rg "$LOCAL_BIN/"
    rm -rf ripgrep-${RIPGREP_VERSION}-x86_64-unknown-linux-musl*
    echo -e "${GREEN}✓ ripgrep installed: $("$LOCAL_BIN/rg" --version)${NC}"
fi

# Install fzf
echo ""
echo -e "${BLUE}========================================${NC}"
echo -e "${BLUE}Step 6/9: Installing fzf ${FZF_VERSION}${NC}"
echo -e "${BLUE}========================================${NC}"
if command -v "$LOCAL_BIN/fzf" &> /dev/null; then
    echo -e "${YELLOW}fzf already installed${NC}"
else
    echo -e "${YELLOW}Downloading fzf binary...${NC}"
    cd "$BUILD_DIR"
    ...
    echo -e "${GREEN}✓ fzf installed: $("$LOCAL_BIN/fzf" --version)${NC}"
fi
...
# Set up manpath for local installations
...
echo -e "  • ripgrep ${RIPGREP_VERSION} → $LOCAL_BIN/rg"
echo -e "  • fzf ${FZF_VERSION} → $LOCAL_BIN/fzf"
...
echo -e "${BLUE}To uninstall: Remove ~/.local/bin/{nvim,lazygit,rg,fzf}, ~/.cargo, ~/.nvm, ~/.opencode, ~/.config/nvim${NC}"
echo ""
